...
i	pr_smb(PR_MISC, "Waiting on falling src det\n");
...
static int smbchg_request_irqs(struct smbchg_chip *chip)
{
	...
	for_each_available_child_of_node(chip->pdev->dev.of_node, child) {
		...
		case SMBCHG_LITE_BAT_IF_SUBTYPE:
			...
			if (rc < 0)
				return rc;
			rc = smbchg_request_irq(chip, child, chip->batt_warm_irq,
				"batt-warm", batt_warm_handler, flags);
			if (rc < 0)
				return rc;
			rc = smbchg_request_irq(chip, child, chip->batt_cool_irq,
				"batt-cool", batt_cool_handler, flags);
			if (rc < 0)
				return rc;
			rc = smbchg_request_irq(chip, child, chip->batt_cold_irq,
				"batt-cold", batt_cold_handler, flags);
			if (rc < 0)
				return rc;
			rc = smbchg_request_irq(chip, child, chip->batt_missing_irq,
				"batt-missing", batt_pres_handler, flags);
			if (rc < 0)
				return rc;
			rc = smbchg_request_irq(chip, child, chip->vbat_low_irq,
